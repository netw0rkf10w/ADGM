...
Matlab call: [x, residuals, rho] = mexADGM_3rdORDER(x0, indH1, valH1, indH2, valH2, indH3, valH3, rhos, MAX_ITER, verb, restart, iter1, iter2, variant)
**************************/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	int* indH1 = (int*)mxGetPr(prhs[1]);
	double* valH1 = mxGetPr(prhs[2]);
	int* indH2 = (int*)mxGetPr(prhs[3]);
	double* valH2 = mxGetPr(prhs[4]);
	int* indH3 = (int*)mxGetPr(prhs[5]);
	double* valH3 = mxGetPr(prhs[6]);
	double* rhos = mxGetPr(prhs[7]);
	int MAX_ITER = mxGetScalar(prhs[8]);
	bool verb = mxGetScalar(prhs[9]);
	bool restart = mxGetScalar(prhs[10]);
	int iter1 = mxGetScalar(prhs[11]);
	int iter2 = mxGetScalar(prhs[12]);
	int variant = mxGetScalar(prhs[13]);
	if(verb)
	...
	int Nt1 = mxGetM(prhs[1]);
	int Nt2 = mxGetM(prhs[3]);
	int Nt3 = mxGetM(prhs[5]);
	int n_rho = max(mxGetN(prhs[7]), mxGetM(prhs[7]));
	...
	int N = ADGM_3rdORDER(x, residuals, rho, x0, n1, n2, indH3, valH3, Nt3,
